...
SIMAPP = ./app
HTTPS_GIT := https://github.com/AstraProtocol/astra.git
DOCKER := $(shell which docker)
DOCKER_BUF := $(DOCKER) run --rm -v $(CURDIR):/workspace --workdir /workspace bufbuild/buf
NAMESPACE := AstraProtocol
PROJECT := astra
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=astra \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(ASTRA_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm astra || true
	# create a new container from the latest image
	$(DOCKER) create --name astra -t -i ${DOCKER_IMAGE}:latest astra
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp astra:/usr/bin/astrad ./build/
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(ASTRA_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(ASTRA_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(ASTRA_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(ASTRA_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
