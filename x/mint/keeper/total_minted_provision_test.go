...
func (suite *KeeperTestSuite) TestSetGetTotalMintProvision() {
	totalMintProvision := sdk.NewDec(1_000_000_000)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				suite.app.MintKeeper.SetTotalMintProvision(suite.ctx, totalMintProvision)
			},
			false,
		},
	}
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			provision := suite.app.MintKeeper.GetTotalMintProvision(suite.ctx)
