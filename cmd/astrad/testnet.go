...
Example:
	astrad testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "astrad", "Home directory of the node's daemon configuration")
...
Example:
	astrad testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			...
			stakingtypes.NewCommissionRates(sdk.MustNewDecFromStr("0.05"), sdk.OneDec(), sdk.OneDec()),
			sdk.OneInt(),
		)
		if err != nil {
			return err
